...
void model_client_t::append_to_timestamp_map(timestamp_map_t &profile_time_stamps, timestamp_t &timestamp, std::string &function_string){
    if (profile_time_stamps.find(function_string) == profile_time_stamps.end())
        ...
        profile_time_stamps[function_string].insert( profile_time_stamps[function_string].end(), {timestamp.first, timestamp.second});
}
            
void model_client_t::append_to_timestamp_map(timestamp_map_t &profile_time_stamps, std::vector<uint64_t> &timestamp, std::string &function_string){
    if (profile_time_stamps.find(function_string) == profile_time_stamps.end())
        profile_time_stamps[function_string] = std::vector<uint64_t>(timestamp.begin(), timestamp.end());
    else
        profile_time_stamps[function_string].insert( profile_time_stamps[function_string].end(), timestamp.begin(), timestamp.end());
}

bool model_client_t::store_meta(const digraph_t &g, const composition_t &comp, const float val_acc, timestamp_map_t &profile_time_stamps) {
...
bool model_client_t::store_layers(const model_id_t &id, const vertex_list_t &layer_id,
                    std::vector<segment_t>&segments, timestamp_map_t &profile_time_stamps){
...
composition_t& model_client_t::get_composition(const model_id_t &id, timestamp_map_t &profile_time_stamps) {
...
bool model_client_t::read_layers(const model_id_t &id, const vertex_list_t &layer_id, std::vector<segment_t> &segment_list, std::vector<uint64_t> &owners, timestamp_map_t &profile_time_stamps){
    struct req_info_t {
...
prefix_t model_client_t::get_prefix(const digraph_t &child, timestamp_map_t &profile_time_stamps) {
    std::string function_str = std::string(__func__);
    prefix_t max_result;
...
int model_client_t::shutdown() {
    std::cout << "client issued shutdown!!" << std::endl;
