...
class model_server_t : public tl::provider<model_server_t> {
    struct model_info_t {
 	...
 	float val_acc;
        model_info_t(const std::vector<digraph_t>::iterator &idx, const composition_t &comp, const float &acc) : index(idx), composition(comp), val_acc(acc) { }
    };
    ...
    std::string policy;
public:
    model_server_t(tl::engine& e, uint16_t provider_id = 0, uint32_t num_procs=1, std::string const& server_policy=std::string("map"));
    ~model_server_t();
