...
timestamp_t model_server_t::store_meta(const digraph_t &g, const composition_t &comp, const float val_acc) {
...
void model_server_t::store_layers(const tl::request &req, const model_id_t &id, const vertex_list_t &layer_id,
    ...
    void *ptr;
    for (int i = 0; i < layer_id.size(); i++) {
        std::unique_lock<tl::mutex> lock(rdma_segments.buffer_lock);
        ptr = rdma_segments.dbg->do_allocate(layer_size[i], alignof(std::max_align_t));
        lock.unlock();
...
std::pair<prefix_t, timestamp_t> model_server_t::get_prefix(const digraph_t &child) {
    ...
    for (auto &parent : graph_store) {
        ...
        if ( ( prefix_len > max_prefix_len ) || ( (prefix_len==max_prefix_len) && (model_accuracy > max_accuracy) ) ) {
...
model_server_t::model_server_t(tl::engine& e, uint16_t provider_id, uint32_t num_procs, std::string const& server_policy)
...
void model_server_t::rdma_buffers_init(tl::engine& e){
	...
	rdma_segments.monotonic_upstream = new std::pmr::monotonic_buffer_resource(rdma_segments.buffer,server_pinned_buffer_size, std::pmr::null_memory_resource());

        std::pmr::pool_options opts;
        rdma_segments.sync_upstream = new std::pmr::unsynchronized_pool_resource(rdma_segments.monotonic_upstream);
        rdma_segments.dbg = new debug_resource("pool", rdma_segments.sync_upstream, rdma_segments.buffer);
}
