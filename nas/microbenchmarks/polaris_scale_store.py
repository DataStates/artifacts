...
workcomm, conn_strs = transfer_method_cls.startup_server(ds_colocated=args.ds_colocated)
...
transfer_method = transfer_method_cls(
        ...
        allow_partial_stores=args.allow_partial_stores
        )

num_stores = 2
num_stores_all = 4*num_stores
perc_transferred_random_trials = np.tile([100,75,50,25], num_stores)
...
for i in range(num_stores_all):
    if i%4== 0:
        models.append(create_model(size_per_layer, args.num_layers, args.variance, 0 ,rank*1000 + i))
        layers_to_store.append([layer.name for layer in models[-1].layers])
    else:
...
store_dict={'25': 0, '50': 0, '75':0, '100':0}
for i in range(num_stores_all):
    workcomm.Barrier()
    t1 = MPI.Wtime()
    partial_store = True
    transfer_method.store(id=rank*1111 + i, model=models[i], prefix=layers_to_store[i], val_acc=0)
    ...
    if workcomm.rank==0:
        ...
        for j in range(1, workcomm_size, 1):
            ...
            if data[1] > max_time:
                max_time = data[1]
        if str(100-perc_transferred_random_trials[i]) == '0':
            store_dict[str(100)] += (t2 - t1)
        else:
            store_dict[str(100-perc_transferred_random_trials[i])] += (t2 - t1)
    else:
...
if workcomm.rank == 0:
    ...
    with open(result_dir + 'ds_store.pickle', 'wb') as handle:
